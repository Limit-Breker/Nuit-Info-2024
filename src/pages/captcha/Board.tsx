...
const Board: React.FC = () => {
    ...
    const handleKeyDown = (e: React.KeyboardEvent<HTMLImageElement>) => {
        ...
        switch (e.key) {
            case "ArrowRight":
                setPosition((prev) => ({ ...prev, x: prev.x + step, y: prev.y - stepHorizontal })); // Déplacement vers la droite
                break;
            case "ArrowLeft":
                setPosition((prev) => ({ ...prev, x: prev.x - step, y: prev.y + stepHorizontal })); // Déplacement vers la gauche
                break;
            default:
                break;
        }
    ...
    useEffect(() => {
        const checkCollision = () => {
            if (image1Ref.current) {
                ...
                // Check collision with all `image2` elements
    ...
    return (
        <div>
            ...
            <div>
                {isColliding ? <h2>Collision Detected!</h2> : <h2>No Collision</h2>}
            </div>
